        ...
        <>
          ...
          {isSheetView ? (
            <div className="overflow-auto max-h-[70vh] mt-4 scrollbar-thin">
              {filteredData.map((cluster, cIndex) => (
                <div key={cluster.clusterId} className="mb-6 border rounded-lg shadow bg-white">
                  {/* Cluster Header */}
                      ...
                      <tbody>
                        {cluster.customers.map((customer, idx) => (
                          ...
                          >
                            ...
                            <td className="px-3 py-2 border text-center">{customer.displayName}</td>
                            <td className="px-3 py-2 border text-center">{customer.qty}</td>
                            <td className="px-3 py-2 border text-center">{customer.size}</td>
                  ...
                  <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    <div className={`${isVisible ? 'col-span-2 overflow-auto max-h-[65vh] scrollbar-thin' : 'col-span-3'}`}>
                      <div className={`grid gap-4 ${isVisible
                        ...
                        {filteredData.slice(0, 7).map((cluster, index) => {
                              ...
                              <Droppable droppableId={`${index}`}>
                                {(provided) => (
                                  <div
                                    ...
                                    className=" p-3 space-y-3 bg-gray-50"
                                  >
                                    {cluster.customers.map((customer, idx) => {
                    ...
                    {isVisible && (
                      <div className={`col-span-1 ${isVisible ? 'block' : 'hidden'} md:block`}>
                        <div className="flex flex-col gap-4">
                          {filteredData.slice(7).map((cluster, index) => {
